...
def train(model, device, loss_fxn, optimizer, data_loader, history, epoch, model_dir):
    ...
    video_label_df = pd.DataFrame(
        {
            ...
            "acc_num": np.concatenate(acc_nums).ravel(),
        }
    )

    patient_label_df = video_label_df.groupby(by=["acc_num"]).agg(
        {"y_true": np.mean, "y_hat": np.mean}
    )
    ...
    current_metrics = pd.DataFrame(
        [[epoch, "train", running_loss / (i + 1), auroc, aupr, acc, b_acc, mcc, pr, re, f1]],
...
def validate(
    ...
    current_metrics = pd.DataFrame(
        [[epoch, "val", running_loss / (i + 1), auroc, aupr, acc, b_acc, mcc, pr, re, f1]],
        columns=history.columns,
    )
    current_metrics.to_csv(
        os.path.join(model_dir, "history.csv"), mode="a", header=False, index=False
    ...
    if val_loss < early_stopping_dict["best_loss"]:
        print(
            f"EARLY STOPPING: Loss has improved from {round(early_stopping_dict['best_loss'], 3)} to {round(val_loss, 3)}! Saving weights."
        )
        ...
        best_model_wts = (
            ...
            else deepcopy(model.module.state_dict())
        )
        torch.save(
            {"weights": best_model_wts, "optimizer": optimizer.state_dict()},
            os.path.join(model_dir, f"chkpt_epoch-{epoch}.pt"),
        )
    else:
        print(
            f"EARLY STOPPING: Loss has not improved from {round(early_stopping_dict['best_loss'], 3)}"
        )
...
def evaluate(model, device, loss_fxn, data_loader, split, classes, history, model_dir, weights):
    ...
    pbar = tqdm.tqdm(
        enumerate(data_loader), total=len(data_loader), desc=f"[{split.upper()}] EVALUATION"
    )
    running_loss = 0.0
    y_true, y_hat = [], []
    acc_nums = []
    ...
    ax.plot(
        history.loc[history["phase"] == "train", "epoch"],
        history.loc[history["phase"] == "train", "loss"],
        label="train",
    )
    ax.plot(
        history.loc[history["phase"] == "val", "epoch"],
        history.loc[history["phase"] == "val", "loss"],
        label="val",
    )
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ...
    ax.plot(
        history.loc[history["phase"] == "train", "epoch"],
        history.loc[history["phase"] == "train", "auroc"],
        label="train",
    )
    ax.plot(
        history.loc[history["phase"] == "val", "epoch"],
        history.loc[history["phase"] == "val", "auroc"],
        label="val",
    )
    ax.set_xlabel("Epoch")
    ax.set_ylabel("AUROC")
    ...
    ax.plot(
        history.loc[history["phase"] == "train", "epoch"],
        history.loc[history["phase"] == "train", "aupr"],
        label="train",
    )
    ax.plot(
        history.loc[history["phase"] == "val", "epoch"],
        history.loc[history["phase"] == "val", "aupr"],
        label="val",
    )
    ax.set_xlabel("Epoch")
    ax.set_ylabel("AUPR")
