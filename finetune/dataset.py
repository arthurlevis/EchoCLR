...
class EchoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if label == "lvh":
            ...
            # Add LVH as column
            self.label_df["lvh"] = self.label_df["acc_num"].map(
                lvh_labels.set_index("Accession Number").to_dict()["LVH_LVMi"]
            )
        ...
        if frac != 1.0:
            ...
            self.label_df = self.label_df[
                self.label_df["acc_num"].isin(
                    np.random.choice(study_ids, size=int(frac * study_ids.size), replace=False)
                )
            ]

            print("Num studies:", int(frac * study_ids.size))
        self.label_df["label"] = self.label_df["av_stenosis"].apply(
    ...
    def _sample_frames(self, x):
        if self.split == "train":
            if x.shape[0] > self.clip_len * self.sampling_rate:
                ...
                x = np.pad(
                    x, ((0, self.clip_len - x.shape[0]), (0, 0), (0, 0), (0, 0)), mode="constant"
                )
                x = np.transpose(x, (3, 0, 1, 2))
        else:
            if x.shape[0] >= self.clip_len * self.sampling_rate + self.num_clips:
                start_indices = np.arange(
                    0,
                    x.shape[0] - self.clip_len * self.sampling_rate,
                    (x.shape[0] - self.clip_len * self.sampling_rate) // self.num_clips,
                )[: self.num_clips]
                x = np.stack(
                    [
                        x[
