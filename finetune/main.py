...
def main(args):
    ...
    if args.model_name == "3dresnet18":
        ...
        if args.ssl != "":
            ...
            weights = torch.load(os.path.join(args.ssl, checkpoints[idx]), map_location="cpu")[
                "weights"
            ]
    ...
    train_dataset = EchoDataset(
        ...
        frac=args.frac,
        kinetics=(args.ssl == "") and (not args.rand_init),
    )
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=args.n_gpu * args.batch_size, shuffle=False, num_workers=4
    )
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=args.n_gpu * args.batch_size, shuffle=False, num_workers=8
    )
    ext_test_loader = torch.utils.data.DataLoader(
        ext_test_dataset, batch_size=args.n_gpu * args.batch_size, shuffle=False, num_workers=8
    )
    ...
    if args.use_class_weights:
        class_weights = compute_class_weight(
            ...
            classes=np.sort(np.unique((train_dataset.label_df["av_stenosis"] == "Severe").values)),
            y=(train_dataset.label_df["av_stenosis"] == "Severe").values,
        )
    ...
    if args.eval_only == "":
        ...
        best_model_wts = None
        while (
            epoch <= args.max_epochs and early_stopping_dict["epochs_no_improve"] <= args.patience
        ):
            history = train(
                model=model,
                device=device,
            ...
            history, early_stopping_dict, best_model_wts = validate(
                model=model,
                device=device,
                loss_fxn=eval_loss_fxn,
                optimizer=optimizer,
        ...
        if args.weight_averaging:
            checkpoints = [
                torch.load(os.path.join(model_dir, f), map_location="cpu")["weights"]
                for f in os.listdir(model_dir)
                if f.endswith(".pt")
            ]
            for key in best_model_wts:
                best_model_wts[key] = torch.stack([chkpt[key] for chkpt in checkpoints], dim=0).sum(
                    dim=0
                ) / len(checkpoints)
    else:
        ...
        idx = np.argmax([int(f.split(".")[0].split("-")[1]) for f in checkpoints])
        best_model_wts = torch.load(
            os.path.join(args.eval_only, checkpoints[idx]), map_location="cpu"
        )["weights"]
    ...
    evaluate(
        ...
        device=device,
        loss_fxn=eval_loss_fxn,
        data_loader=ext_test_loader,
        split="ext_test",
        classes=ext_test_dataset.CLASSES,
        history=history,
        model_dir=model_dir,
        weights=best_model_wts,
    )
